// build tag
console.log("[TW] app build: hoffix3");
...
async function reflect() {
  try {
    ...
    if (!session) {
      ...
      // 공개 랭킹 시도 (실패해도 HTML 기본 플레이스홀더가 남음)
...
async function renderTop5(basis = "bp") {
  ...
  try {
    ...
    console.warn("renderTop5 failed, fallback to placeholders:", e?.message || e);
...
document.getElementById("logoutBtn").addEventListener("click", async () => { try{await supabase.auth.signOut()}catch(e){console.error(e)}; await reflect(); });
